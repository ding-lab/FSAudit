...
parse_args = function() {
    ...
    volume = get_val_arg(args, "-V", NULL)
    do.append = get_bool_arg(args, "-a")
    skip.header = get_bool_arg(args, "-H")
    ...
    val = list( 'in.fn'=in.fn, 'verbose'=verbose, 'out.fn'=out.fn, 'topN'=topN, 'is.gz'=is.gz, 'group.by'=group.by, 'bad.fa'=bad.fs, 'volume'=volume, 
                'do.append'=do.append, 'skip.header'=skip.header)
...
if (! is.null(args$volume)) {
    rFSA$volume = args$volume
}

write.table(rFSA, args$out.fn, append=args$do.append, sep="\t", quote=FALSE, row.names=FALSE, col.names=(!args$skip.header) )
